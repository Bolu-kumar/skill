...
const questionsPerLoad = 5; // Number of questions to load per click of "Next" button

      // Function to fetch the next set of questions from Firebase
      async function fetchNextQuestions() {
        try {
          const snapshot = await firebase.database().ref('excelQuiz').once('value');
          const totalQuestions = snapshot.numChildren(); // Total number of questions in the database
      
          for (let i = 0; i < questionsPerLoad; i++) {
            if (currentQuestionIndex < totalQuestions) {
              const childSnapshot = snapshot.child(currentQuestionIndex);
              const quizData = childSnapshot.val();
              const question = quizData.Question;
              const options = [
                quizData.Option_A,
              ...
              // Handle when all questions are loaded
              break;
            }
          }
        } catch (error) {
      ...
      // Function to load the initial set of questions
      ...
      function load() {
        ...
        questionNumberSpan.innerHTML = currentQuestionIndex + 1;
        questionDiv.innerHTML = questions[currentQuestionIndex].q;
        ...
        options.forEach((option, index) => {
          ...
          optionDiv.classList.add(`option${index + 1}`);
          optionDiv.id = `${index + 1}`;
          optionDiv.textContent = option;
          optionDiv.setAttribute('onclick', 'check(this)');
      ...
      // Function to handle "Next" button click for lazy loading
      function next() {
        load();
